...
const PendingEntriesTable = ({ entries, loading, error, onWeighOut }) => {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="bg-white divide-y divide-slate-200">
            {loading && (
              <tr><td colSpan="6" className="text-center py-8 text-slate-500">Loading pending entries...</td></tr>
            )}
            {error && (
            ...
            {!loading && !error && entries.length > 0 ? entries.map(entry => (
              <tr key={entry.id} className="hover:bg-slate-50">
                ...
                <td className="td text-right font-bold">{entry.gross_weight_tons.toFixed(3)}</td>
              ...
              !loading && !error && (
                <tr><td colSpan="6" className="text-center py-8 text-slate-500">No vehicles are currently pending weigh-out.</td></tr>
              )
            )}
          </tbody>
        </table>
